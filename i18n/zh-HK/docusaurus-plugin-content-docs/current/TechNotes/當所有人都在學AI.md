...
從「用 AI 寫 code」升到「用 AI 根據規格自動生成正確的 code」。聽起來是進步——確實也是某種進步。引申開來也是進步，但它依然沒有問那個根本問題：這段程式應不應該存在？這個系統解決的是真問題還是假問題？壓縮得再完美，方向錯了，只是更高效地走向錯誤。
