---
title: "主題四：微軟的四道緊箍咒——被鎖死的 AI 帝國"
description: "表面上，微軟是 AI 狂潮的最大贏家。但翻開底牌，微軟正被四道「緊箍咒」死死勒住，處境可能比完全沒有雲端算力的 Apple 更加嚴峻。 過去兩年，微軟的 AI 戰略可以用一個詞概括：「鋪入口」。"
summary: "表面上，微軟是 AI 狂潮的最大贏家。但翻開底牌，微軟正被四道「緊箍咒」死死勒住，處境可能比完全沒有雲端算力的 Apple 更加嚴峻。 過去兩年，微軟的 AI 戰略可以用一個詞概括：「鋪入口」。"
---
...
微軟的邏輯是：我佔有全球最大的生產力軟體分發管道，用户一定會買單。
...
| 指標 | 數字 | 意義 |
|---|---|---|
| M365 商業用户 | 4.5 億 | 微軟的生態基盤 |
| M365 Copilot 付費席位 | 1500 萬 | 僅佔商業用户的 **3.3%** |
| 有權限者的實際使用率 | 約 35.8% | 買了的人裏也只有三分之一在用 |
...
**使用率最低的問題：** 用户最大的抱怨是**「體驗碎裂」**——Word 裏的 Copilot 不知道你在 Excel 寫了什麼，每次打開都是一個「失憶」的全新助手。用户需要的是一個**跨應用程式、融入工作流的單一超級大腦**，不是 10 個獨立的 AI 入口。

**更致命的發現：** 獨立研究顯示，當員工同時能使用多個 AI 平台（ChatGPT、Gemini、Copilot）時，**相當比例的活躍用户更偏好非 Copilot 的替代品**。微軟的分發優勢（強制預裝在 Office 裏）並不等於用户忠誠度。
...
**微軟所有旗艦 AI 產品的底層大腦，全部來自 OpenAI 的模型。** 當 OpenAI 開始做 ChatGPT Enterprise 搶微軟的企業客户、開始找 Oracle/Google 分散算力時，微軟的兩難是：

- 繼續依賴 OpenAI → 命脈捏在別人手裏
- 自研模型 → 至少需要 2-3 年，且沒有 OpenAI 十年的數據和研發積累

Suleyman 的「超級智能」部門是微軟的回應，但這是一個五年計劃——在這五年裏，微軟的「靈魂」仍然是借來的。
...
| Google（TPU 集羣） | Linux |
| Meta（AI 訓練集羣） | Linux |
| OpenAI（Azure 上的 GPU 集羣） | Linux |
| Amazon（AWS AI 服務） | Linux |
| 微軟自己（Azure 資料中心） | **超過一半的虛擬機跑 Linux** |

是的，你沒看錯——**連微軟自己的 Azure 雲端，Linux 虛擬機的數量都已經超過 Windows 虛擬機。** 微軟 CEO 納德拉早在 2015 年就公開説過「Microsoft loves Linux」，因為 Azure 的商業增長大部分來自 Linux 工作負載。
...
- 多 GPU 訓練（8張、16張甚至數千張 GPU 串聯的大規模集羣）的通訊協議（NCCL）只在 Linux 上有完整支持
...
從 2005 年深度學習萌芽，到 2012 年 AlexNet 用 GPU 訓練圖像識別，到 2017 年 Transformer 架構誕生——AI 的每一個里程碑都發生在 Linux 環境中。整個 AI 研究社羣的工具鏈、教材、論文附帶的代碼，全部預設 Linux 環境。
...
1. **AI 訓練和推理的核心基礎設施全部運行在 Linux 上。** 不管你是 Google、Meta 還是 OpenAI，數據中心裏的 GPU 集羣跑的都是 Linux。Windows 在這個層面完全不存在。

2. **開發者的工作環境開始脱離 Windows。** AI/ML 工程師的首選工作環境是 macOS（Unix 基因，與 Linux 開發生態無縫相容）或直接用純 Linux。Windows 在 AI 開發者中的市佔率持續下降。

3. **終端 AI 的戰場轉向行動裝置和 Mac。** 本地端跑開源大模型的玩家首選 Apple Silicon（統一記憶體優勢），或在行動裝置上（iOS/Android）。Windows PC 在「個人端 AI」的定位尷尬。

### WSL2：微軟的止血方案

微軟推出 WSL2（Windows Subsystem for Linux）+ GPU 直通技術，本質上是在 Windows 裏面嵌了一個 Linux。這招很聰明——**它不是在解決「Windows 做不了 AI」的問題，而是在防止開發者因為要用 Linux 而直接拋棄 Windows。**

WSL2 確實大幅改善了 Windows 上的 AI 開發體驗：日常開發和中等規模的模型推理已經完全夠用。但它的本質是一個 Hyper-V 虛擬機，存在 `vmmem` 記憶體消耗和跨檔案系統 I/O 效能損失等問題。

更深層的問題是：**WSL2 的成功恰恰證明了 Windows 原生系統的失敗。** 微軟需要在自己的作業系統裏跑另一個作業系統，才能讓開發者留下來——這等於承認 Windows 的核心架構不適合 AI 時代的開發需求。
...
一個幾百億蔘數的開源模型，動輒需要 40-80GB 的 GPU 記憶體才能載入。在傳統 x86 PC 上：
...
![](./Gemini_Generated_Image_ibszioibszioibsz.jpg)