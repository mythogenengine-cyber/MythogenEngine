...
const targetDirs = {
  'zh-Hant': 'docs/TechNotes',
  'zh-HK': 'i18n/zh-HK/docusaurus-plugin-content-docs/current/TechNotes',
  'en': 'i18n/en/docusaurus-plugin-content-docs/current/TechNotes'
};
...
for (const [locale, relativePath] of Object.entries(targetDirs)) {
  ...
  for (const file of files) {
    ...
    if (parsed.data.title) {
      ...
      const slug = parsed.data.slug || `/TechNotes/${id}`;
...
const outPath = path.join(outDir, 'technotes.json');
fs.writeFileSync(outPath, JSON.stringify(outputData, null, 2));

console.log(`Successfully generated technotes metadata at ${outPath}`);
